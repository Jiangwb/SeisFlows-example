...
r=400; % sigma usually equals to 20 times of a grid size
sigma=80; % sigma usually equals to 5 times of a grid size
...
figure(1);plot(Gauss);
%% read data
data=load('model_true_ascii');
[row column]=size(data);
x_max=max(data(:,1));
z_max=max(data(:,2));
% read source location
source=load('sources.dat');
[nsource,~]=size(source);
...
for isource=1:nsource
    for irow=1:row
        dist=sqrt((source_x(isource)-data(irow,1)).^2+(source_z(isource)-data(irow,2)).^2);
        if dist<=2*r-2
...
for i=1:row	
    fprintf(fid,'%f %f %f %f %f\n',data(i,1),data(i,2),mask_final(i,5),mask_final(i,5),mask_final(i,5));
end
fclose(fid);
